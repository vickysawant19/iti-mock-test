import { useState, useEffect, useCallback, useMemo } from "react";
import {
  eachDayOfInterval,
  endOfDay,
  endOfMonth,
  format,
  parse,
  parseISO,
  startOfDay,
  startOfMonth,
  subMonths,
} from "date-fns";
import { toast } from "react-toastify";
import { useSelector } from "react-redux";
import { newAttendanceService } from "@/appwrite/newAttendanceService";
import holidayService from "@/appwrite/holidaysService";
import batchService from "@/appwrite/batchService";
import tradeservice from "@/appwrite/tradedetails";
import { selectActiveBatchId } from "@/store/activeBatchSlice";

const STATUS_ALIASES = {
  p: "present",
  present: "present",
  a: "absent",
  absent: "absent",
  l: "leave",
  leave: "leave",
  onleave: "leave",
  on_leave: "leave",
};

const normalizeStatus = (rawStatus) => {
  const value = String(rawStatus || "")
    .trim()
    .toLowerCase();
  return STATUS_ALIASES[value] || value;
};

const normalizeDate = (value) => {
  if (!value) return null;
  if (typeof value === "string" && /^\d{4}-\d{2}-\d{2}$/.test(value))
    return value;
  try {
    const dateObj =
      typeof value === "string" && value.includes("T")
        ? parseISO(value)
        : new Date(value);
    if (Number.isNaN(dateObj.getTime())) return null;
    return format(dateObj, "yyyy-MM-dd");
  } catch {
    return null;
  }
};

const isSecondOrFourthSaturday = (dateObj) => {
  if (dateObj.getDay() !== 6) return false;
  const weekNumber = Math.ceil(dateObj.getDate() / 7);
  return weekNumber === 2 || weekNumber === 4;
};

const normalizeAttendanceData = (records = []) => {
  const byDate = new Map();
  const invalid = [];

  records.forEach((record) => {
    const date = normalizeDate(record?.date);
    const status = normalizeStatus(record?.status);
    if (!date || !["present", "absent", "leave"].includes(status)) {
      invalid.push(record);
      return;
    }

    const normalized = {
      ...record,
      date,
      status,
      markedAt: record?.markedAt || null,
    };

    const current = byDate.get(date);
    if (!current) {
      byDate.set(date, normalized);
      return;
    }

    const currentTs = new Date(
      current?.$updatedAt || current?.markedAt || current?.$createdAt || 0,
    ).getTime();
    const candidateTs = new Date(
      normalized?.$updatedAt ||
        normalized?.markedAt ||
        normalized?.$createdAt ||
        0,
    ).getTime();
    if (candidateTs >= currentTs) byDate.set(date, normalized);
  });

  return { byDate, records: Array.from(byDate.values()), invalid };
};

const generateWorkingDays = ({ startDate, endDate, holidaysMap }) => {
  if (!startDate || !endDate || startDate > endDate) return [];
  return eachDayOfInterval({ start: startDate, end: endDate })
    .map((dateObj) => ({ dateObj, date: format(dateObj, "yyyy-MM-dd") }))
    .filter(({ dateObj, date }) => {
      if (holidaysMap.has(date)) return false;
      if (dateObj.getDay() === 0) return false;
      if (isSecondOrFourthSaturday(dateObj)) return false;
      return true;
    })
    .map(({ date }) => date);
};

const buildAttendanceMap = ({
  workingDaysList,
  attendanceByDate,
  profile,
  batchId,
}) => {
  const finalMap = new Map();
  workingDaysList.forEach((date) => {
    const existing = attendanceByDate.get(date);
    if (existing) {
      finalMap.set(date, { ...existing, autoGenerated: false });
      return;
    }
    finalMap.set(date, {
      userId: profile?.userId || null,
      batchId: batchId || null,
      tradeId: profile?.tradeId || null,
      date,
      status: "absent",
      markedAt: null,
      remarks: "Auto-marked absent (no record)",
      autoGenerated: true,
    });
  });
  return finalMap;
};

const calculateStats = (records = [], holidayDays = 0) => {
  let presentDays = 0;
  let absentDays = 0;
  let leaveDays = 0;

  records.forEach((record) => {
    if (record.status === "present") presentDays += 1;
    else if (record.status === "leave") leaveDays += 1;
    else if (record.status === "absent") absentDays += 1;
  });

  const totalDays = presentDays + absentDays;
  return {
    totalDays,
    presentDays,
    absentDays,
    leaveDays,
    holidayDays,
    attendancePercentage:
      totalDays > 0 ? Number(((presentDays / totalDays) * 100).toFixed(2)) : 0,
  };
};

const toMinutes = (time, fallback) => {
  const [h, m] = String(time || fallback)
    .split(":")
    .map((v) => Number(v || 0));
  return h * 60 + m;
};

export const useStudentAttendance = (profile) => {
  const activeBatchId = useSelector(selectActiveBatchId);
  const resolvedBatchId = activeBatchId;
  const isResolvingBatch = !activeBatchId && !!profile?.userId;

  const [isLoadingAttendance, setIsLoadingAttendance] = useState(false);
  const [isLoadingOverallStats, setIsLoadingOverallStats] = useState(false);
  const [batchData, setBatchData] = useState(null);
  const [rawMonthlyRecords, setRawMonthlyRecords] = useState([]);
  const [holidays, setHolidays] = useState(new Map());
  const [invalidRecords, setInvalidRecords] = useState([]);
  const [currentMonth, setCurrentMonth] = useState(
    format(new Date(), "MMMM yyyy"),
  );
  const [selectedDate, setSelectedDate] = useState(new Date());
  const [lastUpdatedDate, setLastUpdatedDate] = useState(null);
  const [overallStats, setOverallStats] = useState(null);
  const [tradeData, setTradeData] = useState(null);

  const fetchBatchData = useCallback(async (batchId) => {
    try {
      const data = await batchService.getBatch(batchId);
      setBatchData(data || null);
    } catch (error) {
      console.log(error);
    }
  }, []);

  const fetchTradeData = useCallback(async (tradeId) => {
    try {
      const data = await tradeservice.getTrade(tradeId);
      setTradeData(data || null);
    } catch (error) {
      console.log(error);
    }
  }, []);
  console.log(profile);

  useEffect(() => {
    if (resolvedBatchId) fetchBatchData(resolvedBatchId);
  }, [resolvedBatchId, fetchBatchData]);

  useEffect(() => {
    if (batchData?.tradeId) fetchTradeData(batchData.tradeId);
  }, [batchData?.tradeId, fetchTradeData]);

  const getMonthRange = useCallback((monthLabel) => {
    const parsedDate = parse(monthLabel, "MMMM yyyy", new Date());
    return { start: startOfMonth(parsedDate), end: endOfMonth(parsedDate) };
  }, []);

  const fetchMonthlyAttendance = useCallback(async () => {
    if (!resolvedBatchId || !profile?.userId) return;
    setIsLoadingAttendance(true);
    try {
      const { start, end } = getMonthRange(currentMonth);
      const batchStart = batchData?.start_date
        ? startOfDay(new Date(batchData.start_date))
        : start;
      const batchEnd = batchData?.end_date
        ? endOfDay(new Date(batchData.end_date))
        : endOfDay(end);
      const rangeStart = new Date(
        Math.max(start.getTime(), batchStart.getTime()),
      );
      const rangeEnd = new Date(
        Math.min(
          endOfDay(end).getTime(),
          batchEnd.getTime()
        ),
      );

      const startDate = format(rangeStart, "yyyy-MM-dd");
      const endDate = format(rangeEnd, "yyyy-MM-dd");

      const [attendanceRes, holidayRes] = await Promise.all([
        newAttendanceService.getStudentAttendanceByDateRange(
          profile.userId,
          resolvedBatchId,
          startDate,
          endDate,
        ),
        holidayService.getBatchHolidaysByDateRange(
          resolvedBatchId,
          startDate,
          endDate,
        ),
      ]);

      setRawMonthlyRecords(attendanceRes?.documents || []);
      const holidayMap = new Map();
      (holidayRes || []).forEach((item) => {
        const date = normalizeDate(item?.date);
        if (!date) return;
        if (date > endDate || date < startDate) return;
        holidayMap.set(date, String(item?.holidayText || "Holiday").trim());
      });
      setHolidays(holidayMap);
    } catch (error) {
      console.log(error);
      toast.error("Failed to fetch attendance.");
    } finally {
      setIsLoadingAttendance(false);
    }
  }, [profile, currentMonth, getMonthRange, batchData, resolvedBatchId]);

  useEffect(() => {
    fetchMonthlyAttendance();
  }, [fetchMonthlyAttendance]);

  const fetchOverallStats = useCallback(async () => {
    if (!profile?.userId || !resolvedBatchId) return;
    setIsLoadingOverallStats(true);
    try {
      // Historical Overall = From batch start up to end of previous month.
      // Using currentMonth (not selectedDate) so clicking individual dates within
      // the same month does NOT re-trigger this fetch.
      const batchStart = batchData?.start_date ? batchData.start_date : null;
      const currentMonthDate = parse(currentMonth, "MMMM yyyy", new Date());
      const prevMonthEnd = format(endOfMonth(subMonths(currentMonthDate, 1)), "yyyy-MM-dd");

      const stats = await newAttendanceService.getStudentAttendanceStats(
        profile.userId,
        resolvedBatchId,
        batchStart,
        prevMonthEnd
      );
      setOverallStats({
        totalDays: stats.total,
        presentDays: stats.presentDays,
        absentDays: stats.absentDays,
        attendancePercentage: stats.percentage,
        leaveDays: 0,
        workingDays: stats.total,
      });
    } catch (error) {
      console.error("Error fetching overall stats:", error);
    } finally {
      setIsLoadingOverallStats(false);
    }
  }, [profile, resolvedBatchId, batchData?.start_date, currentMonth]);

  useEffect(() => {
    fetchOverallStats();
  }, [fetchOverallStats]);

  const normalized = useMemo(
    () => normalizeAttendanceData(rawMonthlyRecords),
    [rawMonthlyRecords],
  );

  useEffect(() => {
    setInvalidRecords(normalized.invalid);
  }, [normalized.invalid]);

  const workingDaysList = useMemo(() => {
    if (!batchData?.start_date) return [];
    const { start: monthStart, end: monthEnd } = getMonthRange(currentMonth);
    const batchStart = startOfDay(new Date(batchData.start_date));
    const batchEnd = batchData?.end_date
      ? endOfDay(new Date(batchData.end_date))
      : monthEnd;

    const rangeStart = new Date(
      Math.max(monthStart.getTime(), batchStart.getTime()),
    );
    const rangeEnd = new Date(
      Math.min(monthEnd.getTime(), batchEnd.getTime()),
    );

    return generateWorkingDays({
      startDate: rangeStart,
      endDate: rangeEnd,
      holidaysMap: holidays,
    });
  }, [batchData, currentMonth, getMonthRange, holidays]);

  const finalAttendanceMap = useMemo(
    () =>
      buildAttendanceMap({
        workingDaysList,
        attendanceByDate: normalized.byDate,
        profile,
        batchId: resolvedBatchId,
      }),
    [workingDaysList, normalized.byDate, profile, resolvedBatchId],
  );

  const finalAttendanceRecords = useMemo(
    () =>
      Array.from(finalAttendanceMap.values()).sort(
        (a, b) => new Date(b.date) - new Date(a.date),
      ),
    [finalAttendanceMap],
  );

  const stats = useMemo(() => {
    const todayStr = format(new Date(), "yyyy-MM-dd");
    const recordsUpToToday = finalAttendanceRecords.filter(r => r.date <= todayStr);
    return calculateStats(recordsUpToToday, holidays.size);
  }, [finalAttendanceRecords, holidays.size]);

  const studentAttendance = useMemo(
    () => ({
      ...profile,
      attendanceRecords: finalAttendanceRecords,
      batchId: resolvedBatchId,
    }),
    [profile, finalAttendanceRecords, resolvedBatchId],
  );

  const markAttendance = async (dateStr, status = "present", remarks = "") => {
    try {
      if (!batchData?.canMarkAttendance) {
        toast.warn("Attendance marking is disabled for this batch.");
        return null;
      }
      const normalizedDate = normalizeDate(dateStr);
      if (!normalizedDate) {
        toast.error("Invalid date.");
        return null;
      }

      const today = format(new Date(), "yyyy-MM-dd");
      if (normalizedDate > today) {
        toast.warn("Future dates cannot be marked.");
        return null;
      }
      if (normalizedDate < today && !batchData?.canMarkPrevious) {
        toast.warn("Past attendance marking is disabled.");
        return null;
      }
      if (holidays.has(normalizedDate)) {
        toast.warn("Cannot mark attendance on a holiday.");
        return null;
      }
      if (
        batchData?.start_date &&
        normalizedDate < normalizeDate(batchData.start_date)
      ) {
        toast.warn("Date is before batch start.");
        return null;
      }
      if (
        batchData?.end_date &&
        normalizedDate > normalizeDate(batchData.end_date)
      ) {
        toast.warn("Date is after batch end.");
        return null;
      }

      const nowMins = new Date().getHours() * 60 + new Date().getMinutes();
      const startMins = toMinutes(batchData?.attendanceTime?.start, "09:00");
      const endMins = toMinutes(batchData?.attendanceTime?.end, "18:00");
      if (
        normalizedDate === today &&
        (nowMins < startMins || nowMins > endMins)
      ) {
        toast.warn("Attendance can only be marked within attendance time.");
        return null;
      }

      const existing = normalized.byDate.get(normalizedDate);
      const safeStatus = normalizeStatus(status);

      // Skip API call if the record already has the same status
      if (existing && !existing.autoGenerated && normalizeStatus(existing.status) === safeStatus) {
        toast.info("Attendance already marked as " + safeStatus + ".");
        return existing;
      }

      let result;
      if (existing) {
        result = await newAttendanceService.updateAttendance(existing.$id, {
          status: safeStatus,
          remarks,
          markedBy: profile?.userId,
        });
      } else {
        result = await newAttendanceService.createAttendance({
          userId: profile?.userId,
          batchId: resolvedBatchId,
          tradeId: profile?.tradeId || null,
          date: normalizedDate,
          status: safeStatus,
          remarks,
          markedBy: profile?.userId,
        });
      }

      // Update only the changed date locally to avoid full-month refetch flicker.
      setRawMonthlyRecords((prev) => {
        const next = Array.isArray(prev) ? [...prev] : [];
        const idx = next.findIndex(
          (r) => normalizeDate(r?.date) === normalizedDate,
        );
        const merged = {
          ...(idx >= 0 ? next[idx] : {}),
          ...(result || {}),
          date: normalizedDate,
          status: safeStatus,
          remarks,
          markedBy: profile?.userId,
          markedAt: result?.markedAt || new Date().toISOString(),
        };
        if (idx >= 0) next[idx] = merged;
        else next.push(merged);
        return next;
      });
      setLastUpdatedDate(normalizedDate);
      setTimeout(() => setLastUpdatedDate(null), 2200);

      toast.success("Attendance saved successfully!");
      return result;
    } catch (error) {
      console.error("Error marking attendance:", error);
      toast.error("Failed to mark attendance.");
      throw error;
    }
  };

  // Collect all blank (auto-generated) days for the current month view — limited to past days only
  const blankDays = useMemo(
    () => {
      const todayStr = format(new Date(), "yyyy-MM-dd");
      return Array.from(finalAttendanceMap.entries())
        .filter(([date, rec]) => rec.autoGenerated && date < todayStr)
        .map(([date]) => date)
        .sort();
    },
    [finalAttendanceMap],
  );

  const [isBulkMarking, setIsBulkMarking] = useState(false);

  const markBulkBlank = useCallback(
    async (status = "absent", remarks = "") => {
      if (!batchData?.canMarkAttendance || !batchData?.canMarkPrevious) {
        toast.warn("Bulk marking requires both marking and past-marking settings to be enabled.");
        return;
      }
      if (blankDays.length === 0) {
        toast.info("No blank days to mark.");
        return;
      }
      if (isBulkMarking) return;
      setIsBulkMarking(true);
      try {
        const daysToMark = blankDays; // Already filtered to batchStart <= date <= today
        if (daysToMark.length === 0) {
          toast.info("No past blank days to mark.");
          setIsBulkMarking(false);
          return;
        }
        const records = daysToMark.map((date) => ({
          userId: profile?.userId,
          batchId: resolvedBatchId,
          tradeId: profile?.tradeId || null,
          date,
          status,
          remarks: remarks || `Bulk marked ${status}`,
          markedBy: profile?.userId,
          markedAt: new Date().toISOString(),
        }));

        await newAttendanceService.createMultipleAttendance(records);

        // Patch local state so the calendar reflects immediately
        setRawMonthlyRecords((prev) => {
          const next = Array.isArray(prev) ? [...prev] : [];
          records.forEach((rec) => {
            const idx = next.findIndex(
              (r) => normalizeDate(r?.date) === rec.date,
            );
            if (idx >= 0) next[idx] = { ...next[idx], ...rec };
            else next.push(rec);
          });
          return next;
        });

        toast.success(
          `${daysToMark.length} day${daysToMark.length > 1 ? "s" : ""} marked as ${status}!`,
        );
      } catch (err) {
        console.error("Bulk mark error:", err);
        toast.error("Bulk marking failed.");
      } finally {
        setIsBulkMarking(false);
      }
    },
    [batchData, blankDays, isBulkMarking, profile, resolvedBatchId],
  );

  return {
    isLoadingAttendance: isLoadingAttendance || isResolvingBatch,
    isLoadingOverallStats,
    batchData,
    tradeData,
    studentAttendance,
    holidays,
    workingDays: finalAttendanceMap,
    finalAttendanceMap,
    finalAttendanceRecords,
    workingDaysList,
    attendanceByDate: finalAttendanceMap,
    // Raw DB records keyed by date — includes Sundays, 2nd/4th Saturdays etc.
    // Used by the calendar to display records on non-working days without
    // affecting stats which are calculated from finalAttendanceMap only.
    rawAttendanceByDate: normalized.byDate,
    attendanceStats: stats,
    monthlyStats: stats,
    overallStats,
    currentMonth,
    selectedDate,
    setSelectedDate,
    handleMonthChange: (dateOrStr) => {
      const month =
        typeof dateOrStr === "string"
          ? dateOrStr
          : format(dateOrStr, "MMMM yyyy");
      setCurrentMonth(month);
      if (typeof dateOrStr !== "string") setSelectedDate(dateOrStr);
    },
    markAttendance,
    markBulkBlank,
    isBulkMarking,
    blankDays,
    lastUpdatedDate,
    invalidRecords,
  };
};
