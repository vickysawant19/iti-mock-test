...
import { UserCircle2, IdCard } from "lucide-react";
...
const StudentManagementModal = ({
  ...
  return (
    ...
    >
      <DialogContent className="w-[95vw] sm:max-w-5xl lg:max-w-6xl max-h-[92vh] overflow-y-auto overflow-x-hidden p-0 gap-0 rounded-3xl border border-slate-200/80 dark:border-slate-800 shadow-2xl bg-white dark:bg-slate-900">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 w-full">
            ...
            <div className="flex items-center gap-3 min-w-0">
              <InteractiveAvatar
                ...
                fallbackText={selectedStudent?.userName?.charAt(0) || "S"}
              ...
              <div className="min-w-0">
                <div className="flex items-center gap-2">
                  <DialogTitle className="text-base sm:text-lg font-extrabold text-slate-900 dark:text-white truncate">
                    {selectedStudent?.userName || selectedStudent?.name || "Student Management"}
                  </DialogTitle>
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400 font-medium truncate">
                  Roll No: {selectedStudent?.studentId || selectedStudent?.rollNumber || "N/A"} • {selectedStudent?.email || "No email"}
                </p>
              </div>
            </div>

            {/* Styled Segmented Control Tabs */}
            <div className="flex items-center gap-1 bg-slate-100 dark:bg-slate-800/80 p-1 rounded-xl shadow-inner border border-slate-200/60 dark:border-slate-700/60 shrink-0">
              <button
        ...
        {/* Modal Tab Body */}
        <div className="p-4 sm:p-6 bg-slate-50/50 dark:bg-slate-900/50 min-h-[400px] w-full min-w-0 overflow-x-hidden">
