...
const AddBulkQuestions = () => {
  ...
  const fetchModules = async () => {
    if (!tradeData.selectedTrade || !subjectData.selectedSubject || !selectedTradeYear) {
      return;
    }
    ...
    try {
      ...
      const response = await moduleServices.listModules([
        Query.equal("tradeId", tradeData.selectedTrade.$id),
        Query.equal("subjectId", subjectData.selectedSubject.$id),
        Query.equal("year", selectedTradeYear),
      ]);
      // Extract syllabus array from the first document
      const syllabusData = response?.syllabus || [];
      setModulesData({
        data: syllabusData,
        selectedModule: null
      });
  ...
  const handleJsonInput = (value) => {
    ...
    try {
      ...
      if (!isValid) {
        setJsonError('Invalid question structure. Each question must have: question, correctAnswer, and options array');
        setParsedQuestions([]);
        return;
      }
  ...
  const handleSubmit = async () => {
    if (!tradeData.selectedTrade || !subjectData.selectedSubject || 
        !selectedTradeYear || !modulesData.selectedModule) {
    ...
    try {
      ...
      toast.success(`${parsedQuestions.length} question${parsedQuestions.length !== 1 ? 's' : ''} submitted successfully!`);
      setJsonInput('');
      setParsedQuestions([]);
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-6xl mx-auto">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900">Add Bulk Questions</h1>
        ...
        <div className="bg-white rounded-lg shadow-sm p-6 mb-6">
          <h2 className="text-lg font-semibold text-gray-900 mb-4">Select Criteria</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div>
              ...
              >
                <Select.Trigger className="w-full inline-flex items-center justify-between px-4 py-2 bg-white border border-gray-300 rounded-lg text-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:opacity-50 disabled:cursor-not-allowed">
                  <Select.Value placeholder={tradeData.data.length === 0 ? "No trades available" : "Select trade"} />
                  <Select.Icon>
                  ...
                  <Select.Content className="bg-white rounded-lg shadow-lg border border-gray-200 overflow-hidden z-50">
                    <Select.Viewport className="p-1">
                      {tradeData.data.map((trade) => (
                        ...
                        >
                          <Select.ItemText>{trade.tradeName || trade.name}</Select.ItemText>
            ...
            <div>
              ...
              <Select.Root
                ...
                onValueChange={(value) => {
                  ...
                  setSubjectData(prev => ({ ...prev, selectedSubject: subject }));
                }}
                disabled={fetchingData || subjectData.data.length === 0}
              >
                <Select.Trigger className="w-full inline-flex items-center justify-between px-4 py-2 bg-white border border-gray-300 rounded-lg text-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:opacity-50 disabled:cursor-not-allowed">
                  <Select.Value placeholder={subjectData.data.length === 0 ? "No subjects available" : "Select subject"} />
                  <Select.Icon>
                    <ChevronDown className="w-4 h-4" />
                  </Select.Icon>
                </Select.Trigger>
                    ...
                    <Select.Viewport className="p-1">
                      {subjectData.data.map((subject) => (
                        <Select.Item
                          ...
                          className="relative flex items-center px-8 py-2 text-sm rounded cursor-pointer hover:bg-blue-50 focus:bg-blue-50 outline-none"
                        >
                          <Select.ItemText>{subject.subjectName || subject.name}</Select.ItemText>
                          <Select.ItemIndicator className="absolute left-2">
                            <Check className="w-4 h-4" />
                          </Select.ItemIndicator>
                        </Select.Item>
            ...
            <div>
              ...
              <Select.Root
                ...
                onValueChange={(value) => {
                  const module = modulesData.data.find(m => m.moduleId === value);
                  setModulesData(prev => ({ ...prev, selectedModule: module }));
                }}
              ...
              >
                <Select.Trigger className="w-full inline-flex items-center justify-between px-4 py-2 bg-white border border-gray-300 rounded-lg text-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:opacity-50 disabled:cursor-not-allowed">
                  <Select.Value placeholder={
                    loading ? "Loading..." : 
                    modulesData.data.length === 0 ? "Select criteria first" : 
                    "Select module"
                  } />
                  <Select.Icon>
              ...
              {modulesData.data.length === 0 && !loading && tradeData.selectedTrade && subjectData.selectedSubject && selectedTradeYear && (
                <p className="mt-1 text-xs text-amber-600">No modules found for selected criteria</p>
              )}
            </div>
          </div>

          {/* Selected Summary */}
          {(tradeData.selectedTrade || subjectData.selectedSubject || selectedTradeYear || modulesData.selectedModule) && (
            <div className="mt-4 p-4 bg-blue-50 rounded-lg border border-blue-200">
              <div className="flex items-start">
                <AlertCircle className="w-5 h-5 text-blue-600 mr-2 flex-shrink-0 mt-0.5" />
                <div>
                  <p className="text-sm font-medium text-blue-900 mb-1">Selected Criteria</p>
                  <div className="text-sm text-blue-800 space-y-1">
                    {tradeData.selectedTrade && (
                      <p><span className="font-medium">Trade:</span> {tradeData.selectedTrade.tradeName || tradeData.selectedTrade.name}</p>
                    )}
                    {subjectData.selectedSubject && (
                      <p><span className="font-medium">Subject:</span> {subjectData.selectedSubject.subjectName || subjectData.selectedSubject.name}</p>
                    )}
                    {selectedTradeYear && (
                      <p><span className="font-medium">Year:</span> {selectedTradeYear === 'FIRST' ? 'First Year' : 'Second Year'}</p>
                    )}
                    {modulesData.selectedModule && (
                      <p><span className="font-medium">Module:</span> {modulesData.selectedModule.moduleId} - {modulesData.selectedModule.moduleName}</p>
                    )}
                  </div>
                </div>
              </div>
        ...
        {/* JSON Input */}
        <div className="bg-white rounded-lg shadow-sm p-6 mb-6">
          <h2 className="text-lg font-semibold text-gray-900 mb-4">Questions JSON</h2>
          
          <div className="mb-4">
            <label className="block text-sm font-medium text-gray-700 mb-2">
              Paste your questions JSON here <span className="text-red-500">*</span>
            </label>
            <textarea
              value={jsonInput}
            ...
            <p className="mt-2 text-xs text-gray-500">
              Format: Array of question objects. Each must have: question, correctAnswer, options (array), and optional tags.
            </p>
          </div>
            ...
            <div className="mt-6">
              ...
              <div className="max-h-96 overflow-y-auto border border-gray-200 rounded-lg">
                {parsedQuestions.map((q, idx) => (
                  <div key={idx} className="p-4 border-b border-gray-200 last:border-b-0 hover:bg-gray-50">
                    <div className="flex items-start">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="flex items-center gap-4 text-xs text-gray-500">
                          <span className="bg-gray-100 px-2 py-1 rounded">
                            Correct: <span className="font-medium text-gray-700">{q.correctAnswer}</span>
                          </span>
                          {q.tags && (
                            <span className="bg-gray-100 px-2 py-1 rounded">
                              Tags: <span className="font-medium text-gray-700">{q.tags}</span>
                            </span>
                          )}
                        </div>
                      </div>
          ...
          >
            {submitting ? (
              <>
                ...
                Submitting {parsedQuestions.length} question{parsedQuestions.length !== 1 ? 's' : ''}...
              </>
            ) : (
              <>
                <Upload className="w-5 h-5 mr-2" />
                Submit {parsedQuestions.length} Question{parsedQuestions.length !== 1 ? 's' : ''}
              </>
            )}
          </button>
        </div>
