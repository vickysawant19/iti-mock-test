          ...
          >
            ...
            {monthDates.map((date) => {
              ...
              return (
                <td
                  key={date}
                  className={`py-3 px-2 border border-gray-300 dark:border-gray-700 text-center w-16 min-w-16 max-w-16  ${
                  ...
                  {editStudentId === student.userId && !isHoliday ? (
                    updatingAttendance.get(`${student.userId}-${fullDate}`) ? (
                      <div className="flex justify-center items-center">
                        <LoaderCircle className="animate-spin text-indigo-500 dark:text-indigo-300" />
                      </div>
                    ) : (
                      <button
                        onClick={() =>
