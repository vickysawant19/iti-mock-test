...
// ── Appwrite config (same as other scripts in this project) ──────────────────
...
// ── Main ─────────────────────────────────────────────────────────────────────
async function main() {
    const csvPath = join(__dirname, '..', '..', 'modulesFiles', 'firstYearEM.csv');
    const text = readFileSync(csvPath, 'utf-8');
    const rows = parseCSV(text);

    console.log(`\n📄 Parsed ${rows.length} rows from firstYearEM.csv`);
    console.log(`🎯 Target: ${DB_ID} / ${COL_ID}\n`);
    ...
    for (const row of rows) {
        ...
        // Parse evalutionsPoints — it's stored as a JSON array in CSV
        // The CSV value looks like: ["{\"id\":1,...}","{\"id\":2,...}"]
        // We need to produce an array of JSON strings for Appwrite's array attribute
        let evalPoints = null;
        ...
        if (rawEval) {
            try {
                ...
                if (Array.isArray(parsed)) {
                    // Each element may already be a string or an object
                    evalPoints = parsed.map(item =>
                        typeof item === 'string' ? item : JSON.stringify(item)
                    );
                }
            } catch (_) {
                // fallback: store as single-element array
                evalPoints = [rawEval];
            }
        }
        ...
        // Only add evalutionsPoints if we have data
        if (evalPoints && evalPoints.length > 0) {
            doc.evalutionsPoints = evalPoints;
        }

        try {
            // Use moduleId as the document ID for idempotency
            // (safe to re-run — will error on duplicate, which we catch)
